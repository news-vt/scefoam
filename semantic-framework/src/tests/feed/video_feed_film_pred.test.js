...
const CYCLES        = 30;          // teach loop
const PRED_SRC_IDX  = 2;           // frame‑4 (0‑based)
const PRED_TGT_IDX  = 3;           // we expect frame‑5

const rootDir   = path.resolve(__dirname, '..');                     // src/tests
const dataRoot  = path.join(rootDir, 'test_data', 'video_feed','film_feed');
...
beforeAll(() => {
  ...
  // copy all frames so ImageCodec.embed() can read them by filename
  frames.forEach(fn =>
