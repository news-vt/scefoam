// server.js
/* eslint-disable no-console */

import express from 'express';
import cors from 'cors';
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

import SemanticFramework from './semantic_framework.js';
import TransformerModel from './model.js';
import ImageCodec from './image_controller.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// CLI
const idx = process.argv.indexOf('--port');
const PORT = idx !== -1 ? Number(process.argv[idx + 1]) : 3000;

// DIRS
const publicDir = path.join(__dirname, '..', 'public');
const imgDir = path.join(publicDir, 'images');
const tmpDir = path.join(process.cwd(), '__tmp'); // for temporary storage
const kbFile = path.join(publicDir, `knowledge_base_${PORT}.json`);

for (const d of [publicDir, imgDir, tmpDir]) {
  fs.mkdirSync(d, { recursive: true });
}

// Auto‐create KB file if missing
if (!fs.existsSync(kbFile)) {
  const initialWrapper = {
    map: {},
    receivedData: [],
    hexHistory: [],
    predictedText: '',
    modelReady: false
  };
  fs.writeFileSync(kbFile, JSON.stringify(initialWrapper, null, 2));
  console.log(`✨ Created new knowledge base at ${kbFile}`);
}

const imageEmbedding = new ImageCodec(/* any config if needed */);
const model = new TransformerModel({ kbPath: kbFile, port: PORT });

const sf = new SemanticFramework({
  kbPath: kbFile,
  embedding: imageEmbedding,
  model: model,
  imgDir: imgDir
});

let latestB64 = null;

const persist = () => {
  if (!sf.kbPath) {
    sf.kbPath = kbFile;
  }
  fs.writeFileSync(kbFile, JSON.stringify(sf.exportKB(), null, 2));
};

const app = express();
app.use(cors());
app.use(express.json({ limit: '50mb' }));
app.use(express.static(publicDir));

app.get('/ping', (_q, r) => r.json({ ok: true, port: PORT }));
app.get('/knowledge_base', (_q, r) => {
  const kbJson = JSON.parse(fs.readFileSync(kbFile, 'utf8'));
  kbJson.latestImg = latestB64;          // might be null
  r.json(kbJson);
});

// SEND (teacher)
app.post('/send', async (req, res) => {
  try {
    const { imageData, text } = req.body;
    if (!imageData && !text) {
      return res.status(400).json({ error: 'No payload provided.' });
    }

    let payload;
    if (imageData) {
      // 1) Strip off "data:…;base64," if present, then decode:
      const b64 = imageData.includes(',') ? imageData.split(',').pop() : imageData;
      const jpegBuffer = Buffer.from(b64, 'base64');

      // Log raw JPEG size (in bytes) before encoding to latent:
      const jpegSizeBytes = jpegBuffer.length;
      console.log(`Image-size: ${jpegSizeBytes} bytes`);

      // 2) Encode to latent via ImageCodec server:
      let latentArray;
      try {
        latentArray = await imageEmbedding.embedBytes(jpegBuffer);
      } catch (err) {
        console.error("ImageCodec.embedBytes failed:", err);
        return res.status(500).json({ error: err.message });
      }

      // 3) Pass the raw latent array into sf.send(...) AS A SINGLE ELEMENT:
      payload = sf.send([latentArray]);

    } else {
      // If you also want to support text:
      const vec = sf.encode_vec(text);
      payload = sf.send([vec]);
    }

    // Log payload size (in bytes) to the server console:
    const payloadSizeBytes = Buffer.byteLength(JSON.stringify(payload));
    console.log(`Payload-size: ${payloadSizeBytes} bytes`);

    persist();
    return res.json({ ok: true, payload });

  } catch (e) {
    console.error('/send error:', e);
    return res.status(500).json({ error: e.message });
  }
});

// RECEIVE (apprentice)
app.post('/receive', (req, res) => {
  try {
    const { payload } = req.body;
    if (!Array.isArray(payload)) {
      return res.status(400).json({ error: 'Bad payload: expected an array' });
    }
   sf.receive(payload)
     .then(images => {          // ←  images is the array of Buffers returned
       if (images?.length) {
         latestB64 = 'data:image/png;base64,' + images[0].toString('base64');
       }
        persist();
        res.json({ ok: true });
      })
      .catch(e => {
        console.error('/receive error:', e);
        res.status(500).json({ error: e.message });
      });
  } catch (e) {
    console.error('/receive error:', e);
    res.status(500).json({ error: e.message });
  }
});

// PREDICT
app.post('/predict', (_q, res) => {
  try {
    const lastHex = sf.hexHistory.at(-1);
    if (!lastHex) return res.status(400).json({ error: 'History empty.' });

    const nextHex = sf.predict(lastHex);
    sf.hexHistory.push(nextHex);
    persist();
    res.json({ nextHex, nextText: sf.predictedText });
  } catch (e) {
    console.error('/predict', e);
    res.status(500).json({ error: e.message });
  }
});

// CLEAR
app.post('/clear', (_q, res) => {
  sf.clear();
  persist();
  res.json({ ok: true });
});

// SPA fallback
app.get('*', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.html'));
});

app.listen(PORT, () => {
  console.log(`Dashboard → http://localhost:${PORT}`);
});
